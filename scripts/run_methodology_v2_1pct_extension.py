#!/usr/bin/env python
"""Post-hoc Methodology V2 nominal-1% label-efficiency extension.

This is deliberately outside the sealed Part-5D registry.  It imports the
frozen model/trainer/sampler/reader and the authorized executor's mechanical
helpers, while writing only below results/methodology_v2_1pct_extension.
The nominal 1% selection uses the already-frozen class_rank ordering and the
...
FRACTION_COLUMN = "frac_1"
RESULTS_ROOT = Path(os.environ.get("PCSTE_RESULTS_ROOT", REPO / "results")).expanduser()
RESULTS = RESULTS_ROOT / "methodology_v2_1pct_extension"
LOGS = REPO / "logs/methodology_v2_1pct_extension"
...
def build_onepct(fold: int, seed: int) -> pd.DataFrame:
    df = build_subset_table(fold, seed)
    # Frozen ordering is fraction-independent. ceil() is at least one for
    # every non-empty class, exactly matching the registered rule.
    df[FRACTION_COLUMN] = df["class_rank"] < np.ceil(
        NOMINAL_FRACTION * df["n_class"]).astype(int)
    return df
...
def generate_manifests() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for fold in FOLDS:
        for seed in SEEDS:
            df = build_onepct(fold, seed)
            path = MANIFESTS / f"label_subset_f{fold}_s{seed}_nominal1pct.csv"
...
def dry_run() -> None:
    ...
    for arm in ("S0", "S1"):
        ...
        with torch.no_grad():
            z = trainer.encoder(**{k: v.to(device) for k, v in collated.items()})[
                "global_embedding"]
            assert tuple(z.shape) == (64, 192) and torch.isfinite(z).all()
    ...
    report = {
        "scientific_status": "post-hoc extreme few-shot extension; not pre-registered",
        "label_status": "nominal 1% subset; realized class-wise fraction determined by the minimum-one rule",
        ...
        "ssl_checkpoints": ckpts, "subset_folds": "3/3 PASS",
        ...
        "launch_command": ".venv/bin/python scripts/run_methodology_v2_1pct_extension.py drive",
    }
    ...
    print("=== 1% EXTENSION PRE-LAUNCH AUDIT ===")
    print(f"Host: {report['host']}\nGPU: {report['gpu']}\nGit commit: {report['git_commit']}")
    print("\nDatasets:\nCWRU: PASS\nJNU: PASS\nHIT: PASS\nMAFAULDA: PASS")
    print("\nSSL checkpoints:\n9/9 PASS\n\n1% subsets:\n3/3 folds generated")
    print("\nS0/S1 subset pairing:\n9/9 PASS\n\nRepresentation pipeline:\nPASS")
    print("\nS0 forward/backward:\nPASS\n\nS1 checkpoint load:\nPASS")
    print("\nS1 forward/backward:\nPASS\n\nGPU memory:\nPASS")
    print("\nTest set touched:\nNO\n\nPrimary Methodology V2 files modified:\nNO")
    print("\n============================================================")
    print("       1% EXTENSION READY TO LAUNCH ✅")
    print("============================================================")
...
def run_one(run_id: str, resume: bool = False) -> None:
    ...
    ex.verify_all_seals()
    subset = pd.read_csv(MANIFESTS / f"label_subset_f{fold}_s{seed}_nominal1pct.csv")
    ...
    start_epoch = 0
    cfg = sha_bytes(json.dumps({"status": "POST_HOC", "fraction": .01,
    ...
    state = {"run_id": run_id, "status": "RUNNING", "scientific_status": "POST_HOC",
             "nominal_label_fraction": .01, "fold": fold, "seed": seed,
...
def drive() -> None:
    ...
    for fold in FOLDS:
        for seed in SEEDS:
            for arm in ("s0", "s1"):
                run_one(f"{arm}_f{fold}_s{seed}_l001")
