#!/usr/bin/env python
"""Isolated launcher and strict pre-launch audit for the REGISTERED Methodology V2 10% experiment.

The 10% fraction and all 18 S0/S1 runs are present in the sealed Part-5D registry.  It imports the
frozen model/trainer/sampler/reader and the authorized executor's mechanical
helpers, while writing only below results/methodology_v2_10pct.
The registered 10% selection uses the already-frozen class_rank ordering and the
...
FRACTION_COLUMN = "frac_10"
RESULTS_ROOT = Path(os.environ.get("PCSTE_RESULTS_ROOT", REPO / "results")).expanduser()
RESULTS = RESULTS_ROOT / "methodology_v2_10pct"
LOGS = REPO / "logs/methodology_v2_10pct"
...
def build_tenpct(fold: int, seed: int) -> pd.DataFrame:
...
def generate_manifests() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for fold in FOLDS:
        for seed in SEEDS:
            df = build_tenpct(fold, seed)
            path = MANIFESTS / f"label_subset_f{fold}_s{seed}_registered10pct.csv"
...
def dry_run() -> None:
    ...
    report = {
        "scientific_status": "REGISTERED Methodology V2 10% label-efficiency experiment",
        "label_status": "registered 10% subset; frozen class-aware group-even nested selection",
        ...
        "launch_command": "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True .venv/bin/python scripts/run_methodology_v2_10pct.py drive",
    }
    ...
    print("=== REGISTERED 10% PRE-LAUNCH AUDIT ===")
    print(f"Host: {report['host']}\nGPU: {report['gpu']}\nGit commit: {report['git_commit']}")
    print("\nRegistered fraction 0.10:\nPASS")
    print("\nDatasets:\nCWRU: PASS\nJNU: PASS\nHIT: PASS\nMAFAULDA: PASS")
    print("\nSSL checkpoints:\n9/9 PASS\n\n10% subset manifests:\n9/9 PASS")
    ...
    print("\n============================================================")
    print("       REGISTERED 10% EXPERIMENT READY TO LAUNCH ✅")
    print("============================================================")
...
def run_one(run_id: str, resume: bool = False) -> None:
    ...
    ex.verify_all_seals()
    subset = pd.read_csv(MANIFESTS / f"label_subset_f{fold}_s{seed}_registered10pct.csv")
...
def drive() -> None:
    ...
    for fold in FOLDS:
        for seed in SEEDS:
            for arm in ("s0", "s1"):
                run_one(f"{arm}_f{fold}_s{seed}_l010")
