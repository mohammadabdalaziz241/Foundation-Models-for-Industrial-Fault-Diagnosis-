...
The S0/S1/K1/Q8 models were trained and evaluated in the executed four-domain protocol. The dissertation reports Macro-4, the equal mean of CWRU, JNU, HIT and MaFaulDa Macro-F1. Macro-3, averaging JNU, HIT and MaFaulDa only, is retained as a historical secondary aggregate. Both are computed from the same four-domain results.
...
Original compact classification/AUC summaries are under `tables/low_label/`; original nine-cell pairing proofs are under `../methodology_v2/low_label_provenance/`. The 1% path was executed after the primary grid for scheduling reasons. The condition belongs to the registered reduced-label design, and its historical launcher and directory names are retained unchanged so that recorded paths remain valid.Historical and publication hashes and all portability edits are recorded in `docs/low_label_provenance.md`. No values were recomputed and no heavy artifacts were included.
